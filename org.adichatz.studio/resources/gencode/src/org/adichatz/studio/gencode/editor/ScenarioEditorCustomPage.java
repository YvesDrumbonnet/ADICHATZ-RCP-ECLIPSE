...
import org.adichatz.studio.command.CustomScenarioMergeItem;
...
public class ScenarioEditorCustomPage extends AFormPageCore {
	...
	// the CustomScenarioMergeItem mergeScenarioPGTI.
	private CustomScenarioMergeItem mergeScenarioPGTI;
	...
	public void createContents(){
		...
		fileGroupPGRP = new PGroupController("fileGroup", compositeWrapper$$6CMPS, this) {
			...
			public void createControl() {
				...
				if (isValid()) {
					...
					getComposite().setBackground(AdichatzApplication.getInstance().getFormToolkit().getColors().getColor(IFormColors.H_GRADIENT_START));
		...
		fileLBLLBL = new LabelController("fileLBL", fileGroupPGRP, this) {
			...
			public void createControl() {
				...
				if (isValid()) {
					...
					setBackground(AdichatzApplication.getInstance().getFormToolkit().getColors().getColor(IFormColors.H_GRADIENT_START));
				}
			}
		};
		...
		// Creates control for CustomScenarioMergeItem mergeScenarioPGTI
		mergeScenarioPGTI = new CustomScenarioMergeItem("mergeScenario", fileGroupPGRP, this) {
			@Override
			public void createControl() {
				super.createControl();
				if (isValid()) {
