...
public class ScenarioEditorParametersItem extends EntityManagerCore {
	protected ATabularCore scenarioEditorParamsTable;
	/**
	 * Creates ScenarioEditorParametersItem.
	 *
	 * @param context
	 ...
	 */
	public ScenarioEditorParametersItem(final AdiContext context, IContainerController parentController) {
		super(context);
		// do not create contents when context is null because that means that class is invoked from dynamic part
		if (null != context) {
			coreController = new CTabItemController("parametersItem", parentController, this) {
				@Override
				...
				public void createControl() {
					super.createControl();
					if (isValid()) {
						getItem().setText(AdichatzApplication.getInstance().getMessage("org.adichatz.studio", "adichatzStudio", "scenario.parameters"));
					}
				}
				@Override
				public EntityInjection getEntityInjection() {
					if (null == entityInjection)
						entityInjection = new EntityInjection(this, getDataAccess().getDataCache().fetchEntity(((ScenarioTree) parentController.getEntity().getBean()).getParams()));
	...
	public void createContents(){
		...
		// loads and instantiates the class ScenarioEditorParamsTable.
		scenarioEditorParamsTable  = new ScenarioEditorParamsTable(context, coreController);
	}
}