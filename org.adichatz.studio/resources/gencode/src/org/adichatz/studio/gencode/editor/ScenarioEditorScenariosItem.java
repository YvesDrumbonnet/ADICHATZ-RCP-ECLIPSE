...
public class ScenarioEditorScenariosItem extends EntityManagerCore {
	protected ATabularCore scenarioEditorScenariosTable;
	/**
	 * Creates ScenarioEditorScenariosItem.
	 *
	 * @param context
	 ...
	 */
	public ScenarioEditorScenariosItem(final AdiContext context, IContainerController parentController) {
		super(context);
		// do not create contents when context is null because that means that class is invoked from dynamic part
		if (null != context) {
			coreController = new CTabItemController("scenariosItem", parentController, this) {
				@Override
				...
				public void createControl() {
					super.createControl();
					if (isValid()) {
						getItem().setText(AdichatzApplication.getInstance().getMessage("org.adichatz.studio", "adichatzStudio", "scenario.scenarios"));
					}
				}
				@Override
				public EntityInjection getEntityInjection() {
					if (null == entityInjection)
						entityInjection = new EntityInjection(this, getDataAccess().getDataCache().fetchEntity(((ScenarioTree) parentController.getEntity().getBean()).getScenarios()));
	...
	public void createContents(){
		...
		// loads and instantiates the class ScenarioEditorScenariosTable.
		scenarioEditorScenariosTable  = new ScenarioEditorScenariosTable(context, coreController);
	}
}