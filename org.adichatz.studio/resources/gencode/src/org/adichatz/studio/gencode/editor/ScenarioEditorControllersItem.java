...
public class ScenarioEditorControllersItem extends EntityManagerCore {
	protected ATabularCore scenarioEditorControllersTable;
	/**
	 * Creates ScenarioEditorControllersItem.
	 *
	 * @param context
	 ...
	 */
	public ScenarioEditorControllersItem(final AdiContext context, IContainerController parentController) {
		super(context);
		// do not create contents when context is null because that means that class is invoked from dynamic part
		if (null != context) {
			coreController = new CTabItemController("controllersItem", parentController, this) {
				@Override
				...
				public void createControl() {
					super.createControl();
					if (isValid()) {
						getItem().setText(AdichatzApplication.getInstance().getMessage("org.adichatz.studio", "adichatzStudio", "scenario.controllers"));
					}
				}
				@Override
				public EntityInjection getEntityInjection() {
					if (null == entityInjection)
						entityInjection = new EntityInjection(this, getDataAccess().getDataCache().fetchEntity(((ScenarioTree) parentController.getEntity().getBean()).getControllers()));
	...
	public void createContents(){
		...
		// loads and instantiates the class ScenarioEditorControllersTable.
		scenarioEditorControllersTable  = new ScenarioEditorControllersTable(context, coreController);
	}
}