 ...
 * </p>
 * 
 * Instances of this class represent an individual shelf. A shelf is a container widget similar to a tab folder but with a list-like
 * visualization.
 * <p>
...
public class PShelfItem extends Item {
	...
	/**
	 * Constructs a new instance of this class given its parent (which must be a <code>PShelf</code>), a style value describing its
	 * behavior and appearance, and the index at which to place it in the items maintained by its parent.
	 * <p>
	 ...
	 *                <li>ERROR_INVALID_RANGE - if the index is not between 0 and the number of elements in the parent (inclusive)
	 *                </li>
	 *                </ul>
	...
	/**
	 * Constructs a new instance of this class given its parent (which must be a <code>PShelf</code>) and a style value describing
	 * its behavior and appearance. The item is added to the end of the items maintained by its parent.
	 * <p>
	 * The style value is either one of the style constants defined in class <code>SWT</code> which is applicable to instances of
	 * this class, or must be built by <em>bitwise OR</em>'ing together (that is, using the <code>int</code> "|" operator) two or
	 * more of those <code>SWT</code> style constants. The class description lists the style constants that are applicable to the
