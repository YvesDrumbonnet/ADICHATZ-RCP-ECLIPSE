...
/**
 * <p>
 * NOTE: THIS WIDGET AND ITS API ARE STILL UNDER DEVELOPMENT. THIS IS A PRE-RELEASE ALPHA VERSION. USERS SHOULD EXPECT API CHANGES
 * IN FUTURE VERSIONS.
 * </p>
 * 
 * Instances of this class implement a selectable accordion metaphor, where each shelf contains a client area.
 * <p>
 * The item children that may be added to instances of this class must be of type <code>PShelfItem</code>. <code>Control</code>
 * children are created on the body composite of each items accessed via <code>PShelfItem#getBody</code>.
 * </p>
 * <p>
 * <dl>
 * <dt><b>Styles:</b></dt>
...
public class PShelf extends Canvas {
	...
	private void animateOpenFromTop(PShelfItem previousItem, PShelfItem newItem) {
		...
		while (percentOfWork < 1) {
			...
			int yBottom = getClientArea().y + getClientArea().height
					- (itemHeight * (items.size() - (items.indexOf(previousItem) + 1)));
	...
	/**
	 * Returns the <code>PShelfItem</code> that is currently selected in the receiver.
	 * 
	...
	/**
	 * Returns an array of <code>PShelfItem</code>s which are the items in the receiver.
	 * <p>
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when the receiver's selection changes, by sending it
	 * one of the messages defined in the <code>SelectionListener</code> interface.
	 * <p>
	 * When <code>widgetSelected</code> is called, the item field of the event object is valid. <code>widgetDefaultSelected</code>
	 * is not called.
	 * </p>
	 *
	 * @param listener
	...
	// BEGIN: Added by Adichatz
	private ArrayList<PShelfItem> allItems = new ArrayList<PShelfItem>();

	public void computeItems() {
		if (!openItem.isVisible())
			for (PShelfItem item : allItems)
				if (item.isVisible()) {
					openItem(item, false);
					break;
				}
