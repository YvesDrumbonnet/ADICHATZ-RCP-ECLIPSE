...
/**
 * Formatted text viewer. Add formating capabilities to the <code>Text</code> widget of SWT. This control works on the same
 * principle than the JFace viewers. The embedded text widget is accessible by the getControl() method, allowing to apply to it all
 * necessary behaviors (layout, listeners...).
 * <p>
 * 
 * Formatting is delegated to formatter objects implementing the <code>ITextFormatter</code> interface. Each formatter class manages
 * a base class of values (date, number...).<br>
 * Formatters are associated by 2 different means :
 * <ul>
 * <li>By the <code>setFormatter()</code> method.</li>
 * <li>When <code>setValue()</code> is called and there is currently no formatter, a new one is automatically created based on the
 * class of the value.</li>
 * </ul>
 * 
...
public class FormattedText {
	...
	/**
	 * Creates a formatted text on a newly-created text control under the given parent. The text control is created using the SWT
	 * style bits <code>BORDER</code>.
	 * 
	 * @param parent
	 *            the parent control
	 */
	...
	/**
	 * Creates a formatted text on a newly-created text control under the given parent. The text control is created using the given
	 * SWT style bits.
	 * 
	 * @param parent
	 *            the parent control
	 ...
	 * <p>
	 * 
	 * The returned value is provided by the formatter and is of the type managed by the formatter. For example a
	 * <code>DateFormatter</code> will return a <code>Date</code> value.<br>
	 * If no formatter is associated, the <code>String</code> contained in the <code>Text</code> widget is returned.
	 * 
	...
	/**
	 * Returns the type of value the {@link ITextFormatter} associated with this FormattedText handles, i.e. returns in
	 * {@link #getValue()}.
	 * 
	...
	/**
	 * Returns <code>true</code> if the current value is empty, else <code>false</code>.<br>
	 * An empty value depends of the formatter applied on the Text widget and is not just an empty String in the widget. Formatters
	 * can use special formatting characters to format the value. These characters are not always considered as part of the value.
	 * For example, in a DateFormatter the pattern uses "/" separator and always displays it in the input field.
	 * 
	...
	/**
	 * Set the beep sound to ON or OFF for all the FormattedText fields. Beep sound are produced by formatters on every input error.
	 * Formatter implementation must check this flag on before to emit a beep sound.
	 * 
	 * @param beepSound
	 ...
	 * Parameter can not be null. In some situations, the FormattedText component must not do formatting (eg. when reusing the same
	 * object for editing of different types of values). In this case, use a StringFormatter. This formatter do no formatting.
	 * 
	 * @param formatter
	 *            formatter
	 */
	 ...
	 * <p>
	 * 
	 * If no formatter is currently associated to he widget, a new one is created by the factory based on the value's class.<br>
	 * If the value is incompatible with the formatter, an <code>IllegalArgumentException</code> is returned.
	 * 
	 * @param value
	 *            new value
	 */
	public void setValue(Object value) {
		if (formatter == null) {
			// BEGIN: Added by Adichatz
			throw new RuntimeException("formatter is mandatory for FormattedText");
			// setFormatter(DefaultFormatterFactory.createFormatter(value));
			// END: Added by Adichatz
		}
