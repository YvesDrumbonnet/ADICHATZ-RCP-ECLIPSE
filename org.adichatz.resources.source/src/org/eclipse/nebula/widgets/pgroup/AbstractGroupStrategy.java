 ...
 * <P>
 * The AbstractGroupStrategy handles most behavior for you. All that is required of extending classes, is to implement painting and
 * sizing.
 *
 * @author chris
 */
public abstract class AbstractGroupStrategy {
	...
	protected PGroup group; // replace private by protected (see AdiGroupStrategy which extends RectangleGroupStrategy)
	...
	public boolean isToggleLocation(int x, int y) {
		if (getGroup().getToggleRenderer() != null) {
			Rectangle r = new Rectangle(getGroup().getToggleRenderer().getBounds().x, getGroup().getToggleRenderer().getBounds().y,
					getGroup().getToggleRenderer().getBounds().width, getGroup().getToggleRenderer().getBounds().height);
			if (r.contains(x, y)) {
				return true;
			}
		}
