...
public class RectangleGroupStrategy extends AbstractGroupStrategy {
	...
	public void paint(GC gc) {
		...
		if (getGroup().getToggleRenderer() != null) {
			...
			if ((getGroup().getTogglePosition() & SWT.LEAD) != 0) {
				...
				getGroup().getToggleRenderer().setLocation(new Point(getGroup().getSize().x - hMargin - p.x, toggleY));
			}
		}
		...
		if ((getGroup().getImagePosition() & SWT.TOP) == 0 || image == null) {
			...
			if (gradientColors != null) {
				GraphicUtils.fillGradientRectangle(gc, 0, 0, getGroup().getSize().x, Math.max(titleHeight, toggleHeight),
						gradientColors, gradientPercents, gradientVertical);
			} else {
				gc.fillRectangle(0, 0, getGroup().getSize().x, Math.max(titleHeight, toggleHeight));
				GraphicUtils.fillRoundRectangle(gc, 0, 0, getGroup().getSize().x, Math.max(titleHeight, toggleHeight), null, true,
						!getGroup().getExpanded());
			}
			if ((getGroup().getStyle() & SWT.SMOOTH) != 0) {
				GraphicUtils.drawRoundRectangle(gc, 0, 0, getGroup().getSize().x, Math.max(titleHeight, toggleHeight), null, null,
						true, !getGroup().getExpanded());
			}
		} else {
			...
			if (gradientColors != null) {
				GraphicUtils.fillGradientRectangle(gc, 0, titleHeight - titleAreaHeight, getGroup().getSize().x,
						Math.max(titleAreaHeight, toggleHeight), gradientColors, gradientPercents, gradientVertical);
			} else {
				gc.setBackground(getGroup().getBackground());
				gc.fillRectangle(0, titleHeight - titleAreaHeight, getGroup().getSize().x, Math.max(titleAreaHeight, toggleHeight));
			}
			...
			if ((getGroup().getImagePosition() & SWT.LEAD) != 0) {
				if (getGroup().getToggleRenderer() != null) {
					if (getGroup().getTogglePosition() == SWT.LEAD) {
						imagePoint.x = hMargin + getGroup().getToggleRenderer().getSize().x + betweenSpacing;
					} else {
				...
				if (getGroup().getToggleRenderer() != null) {
					if ((getGroup().getTogglePosition() & SWT.LEAD) != 0) {
						...
						imagePoint.x = getGroup().getSize().x
								- (hMargin + image.getBounds().width + getGroup().getToggleRenderer().getSize().x + betweenSpacing);
					}
				} else {
					imagePoint.x = getGroup().getSize().x - (hMargin + image.getBounds().width);
				}
			}
			if ((getGroup().getImagePosition() & SWT.TOP) == 0 && image.getImageData().height > titleHeight) {
		...
		if (getGroup().getToolItems().length > 0 && getGroup().getToolItemRenderer() != null) {
			...
			if (min) {
				toolItemArea = new Rectangle(x, titleHeight - titleAreaHeight + 2, minSize.x,
						Math.max(titleAreaHeight, toggleHeight) - 4);
			} else {
				toolItemArea = new Rectangle(x, titleHeight - titleAreaHeight + 2, size.x,
						Math.max(titleAreaHeight, toggleHeight) - 4);
			}
		}
		...
		if (!getGroup().getExpanded()) {
			...
			if ((getGroup().getStyle() & SWT.SMOOTH) != 0) {
				...
				GraphicUtils.drawRoundRectangle(gc, 0, titleHeight, getGroup().getSize().x - 1,
						getGroup().getSize().y - titleHeight, null, false, true);

				reg.dispose();
				gc.setClipping((Region) null);
			} else {
				gc.setForeground(_borderColor);
				gc.drawRectangle(0, titleHeight, getGroup().getBounds().width - 1, getGroup().getBounds().height - titleHeight - 1);
			}
		}
	...
	protected Rectangle getTextBounds() {
		...
		if (image != null && !((getGroup().getImagePosition() & SWT.TOP) == 0)) {
			...
			if (getGroup().getToggleRenderer() != null) {
				titleAreaHeight = Math.max(titleAreaHeight, getGroup().getToggleRenderer().getSize().y + (2 * vMargin));
			}
		}
		textPoint.x = hMargin;
		if (image != null) {
	 ...
	 * <p>
	 * For example, to draw a gradient that varies from dark blue to blue and then to white and stays white for the right half of
	 * the label, use the following call to setBackground:
	 * </p>
	 *
	 * <pre>
	 * setBackground(
	 ...
	 *
	 * @param colors
	 *            an array of Color that specifies the colors to appear in the gradient in order of appearance from left to right;
	 *            The value <code>null</code> clears the background gradient; the value <code>null</code> can be used inside the
	 *            array of Color to specify the background color.
	 * @param percents
	 *            an array of integers between 0 and 100 specifying the percent of the width of the widget at which the color should
	 *            change; the size of the percents array must be one less than the size of the colors array.
	 */
	 ...
	 * Specify a gradient of colours to be drawn in the background of the group.
	 * <p>
	 * For example, to draw a gradient that varies from dark blue to white in the vertical, direction use the following call to
	 * setBackground:
	 * </p>
	 *
	 * <pre>
	 * setBackground(new Color[] { display.getSystemColor(SWT.COLOR_DARK_BLUE), display.getSystemColor(SWT.COLOR_WHITE) },
	 * 		new int[] { 100 }, true);
	 * </pre>
	 *
	 * @param colors
	 *            an array of Color that specifies the colors to appear in the gradient in order of appearance from left/top to
	 *            right/bottom; The value <code>null</code> clears the background gradient; the value <code>null</code> can be used
	 *            inside the array of Color to specify the background color.
	 * @param percents
	 *            an array of integers between 0 and 100 specifying the percent of the width/height of the widget at which the color
	 *            should change; the size of the percents array must be one less than the size of the colors array.
	 * @param vertical
