 ...
 * <p/>
 * This implementation using the pure SWT, no UI AWT package is used. For convenience, I put everything into one class. However, the
 * best way to implement this is to use inheritance to create multiple hierarchies.
 * 
...
public class SWTImageCanvas extends Canvas {
	...
	public SWTImageCanvas(final Composite parent, int style) {
		super(parent, style);
		final ControlAdapter controlAdapter = new ControlAdapter() { /* resize listener. */
			public void controlResized(ControlEvent event) {
				syncScrollBars();
			}
		};
		addControlListener(controlAdapter);
		final PaintListener paintListener = new PaintListener() { /* paint listener. */
	...
	private void paint(GC gc) {
		...
		if (sourceImage != null) {
			...
			if (null != imageBackground) {
				newGC.setBackground(imageBackground);
				newGC.fillRectangle(imageRect);
			}
			newGC.setClipping(clientRect);
			newGC.drawImage(sourceImage, imageRect.x, imageRect.y, imageRect.width, imageRect.height, destRect.x, destRect.y,
	...
	/**
	 * Synchronize the scrollbar with the image. If the transform is out of range, it will correct it. This function considers only
	 * following factors :<b> transform, image size, client area</b>.
	 */
