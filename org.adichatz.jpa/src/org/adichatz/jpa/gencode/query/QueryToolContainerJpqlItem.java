	...
	public EditableFormTextController createJpqlOrder(ControllerCore genCode) {
		...
		jpqlOrderEFT = new EditableFormTextController("jpqlOrder", getParentController(coreController), genCode) {
			...
			public void initialize() {
				AControlListener jpqlOrderLSTN$0 = new AControlListener(null, IEventType.AFTER_CREATE_CONTROL) {
					@Override
					public void handleEvent(AdiEvent event) {
						((QueryToolInput) ((AWidgetController) getFromRegister("queryTool")).getEntity().getBean()).getTabularController().addListener(new AControlListener("#POST_REFRESH_TABULAR#", IEventType.REFRESH) {
						    @Override
						    public void handleEvent(AdiEvent event) {
			...
			public void createControl() {
				...
				if (isValid()) {
					...
					formText.addColor("jpql", AdichatzApplication.getInstance().getFormToolkit().getColors().getColor(IFormColors.TB_TOGGLE_HOVER));
					formText.addColor("suffix", AdichatzApplication.getInstance().getFormToolkit().getColors().getColor(IFormColors.TB_TOGGLE));
					formText.addColor("name", AdichatzApplication.getInstance().getFormToolkit().getColors().getColor(IFormColors.H_BOTTOM_KEYLINE2));
				}
			}
		};
		return jpqlOrderEFT;
